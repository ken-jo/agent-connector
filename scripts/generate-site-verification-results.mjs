...
const KNOWN_RESULTS = new Set([
  ...
  "MCP_INSTALL_VERIFIED_AUTH_BLOCKED",
  "INSTALL_DOCTOR_VERIFIED_NO_HEADLESS_MODE",
  "ADAPTER_PLACEMENT_VERIFIED",
]);
for (const row of rows) {
  if (!KNOWN_RESULTS.has(row.result)) {
    throw new Error(`[verification] ${row.host}: unknown result "${row.result}" — add it to the ladder or fix the CSV`);
  }
}
