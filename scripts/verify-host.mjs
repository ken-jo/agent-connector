...
const HOST_LANES = {
  ...
  codex: {
    ...
    accept: { argv: ["mcp", "list"], kind: "list-id" },
    // The OFFLINE accept lane stays list-id. A turn is auth-gated, but codex IS
    // an AUTHED-RUNTIME deep-verb host (see HOST_VERBS["codex"] + codexRunners):
    // with ~/.codex/auth.json copied into the sandbox CODEX_HOME, `codex exec
    // --dangerously-bypass-hook-trust` advances a turn and ALL 5 hooks fire +
    // MCP/telemetry round-trip (env-pinned to survive codex's MCP-child env
    // strip). The bypass-hook-trust flag is MANDATORY — without it hooks fire
    // ZERO, silently (codex requires persisted hook trust).
    note: "`codex mcp list` lists our server offline (live-verified); turn is auth-gated. AUTHED-RUNTIME deep-verb host: with the copied ~/.codex/auth.json, `--verb`/`--all-verbs` fire hooks (requires --dangerously-bypass-hook-trust) + MCP/telemetry live.",
  },
  "gemini-cli": {
    bin: "gemini",
...
/**
 * Codex variant of {@link genHookMcpConnector}: identical hook handlers + REAL
 * echo server, PLUS an env-pinned `server.env` table. THE CODEX-ONLY FIX (see
 * the codex newBug): codex STRIPS the environment of MCP-server children, so the
 * spawned `agent-connector serve` wrapper (and the echo grandchild) do NOT
 * inherit AGENT_CONNECTOR_DATA_DIR / AC_MCP_LOG / AC_TOOL_MARK_DIR from the codex
 * process env. The serve child then resolves the DEFAULT data root
 * ($HOME/.agent-connector), can't find the registry record install wrote under
 * the sandbox-overridden root, and dies "Connector <id> is not registered" —
 * telemetry stays empty + the MCP tool never loads. Codex DOES write+pass the
 * `[mcp_servers.<id>.env]` TOML table (renderMcpEntry → entry.env), and the
 * adapter resolves `${env:VAR}` to a LITERAL at install time, so pinning the
 * three vars here threads the overridden data-root/log paths through the strip.
 * (On main, `serve` has no `--data-dir` flag yet — #231 — so the env-pin is the
 * live-verified mechanism; it is also harmless once that flag lands.)
 */
function genCodexHookMcpConnector({ id, denyTool, telemetry }) {
  const fixture = JSON.stringify(MCP_ECHO_FIXTURE);
  const body = `// GENERATED by verify-host.mjs codex deep-verb lane — do not edit by hand.
import { appendFileSync, mkdirSync } from "node:fs";
import { join } from "node:path";
import { defineConnector } from "@ken-jo/agent-connector";
...
export default defineConnector({
  id: ${JSON.stringify(id)},
  displayName: "AC codex deep-verb verify",
  version: "1.0.0",
  server: {
    transport: "stdio",
    command: process.execPath,
    ...
    tools: { include: ["*"] },
    timeoutMs: 10_000,
    // Pin the data-root + oracle-log paths into the [mcp_servers.<id>.env] table
    // (codex strips MCP-child env; this table survives the strip — see the doc).
    env: {
      AGENT_CONNECTOR_DATA_DIR: "\${env:AGENT_CONNECTOR_DATA_DIR}",
      AC_MCP_LOG: "\${env:AC_MCP_LOG}",
      AC_TOOL_MARK_DIR: "\${env:AC_TOOL_MARK_DIR}",
    },
  },
  hooks: {
    PreToolUse: { async handler(evt) {
...
`;
  return writeConnector(`deep-codex-${id}.config.mjs`, body);
}
...
/** codex: an AUTHED-RUNTIME json-stdio host (config.toml + hooks.json under
 *  $CODEX_HOME). A turn needs the real ChatGPT/API login, so the sandbox COPIES
 *  ~/.codex/auth.json into the isolated CODEX_HOME (lane.env CODEX_HOME=[.codex])
 *  and leaves the rest of the config tree EMPTY so OUR config.toml + hooks.json
 *  are written fresh by install. copyFileSync is a pure read of the real tree →
 *  the real ~/.codex is never written (callers re-assert via sha256 where the
 *  driver compares before/after). Returns { ok, reason } — ok:false is a SKIP
 *  (codex not logged in on this box). */
function prepCodexSandbox(home) {
  const realCodex = join(realHome(), ".codex");
  const realAuth = join(realCodex, "auth.json");
  if (!existsSync(realAuth)) {
    return { ok: false, reason: "no ~/.codex/auth.json — codex not logged in on this box" };
  }
  const sandCodex = join(home, ".codex");
  mkdirSync(sandCodex, { recursive: true });
  copyFileSync(realAuth, join(sandCodex, "auth.json"));
  return { ok: true, reason: "codex sandbox ready (copied ~/.codex/auth.json; real tree untouched)" };
}
...
// ── codex (codex-cli) verb runners ────────────────────────────────────────
// AUTHED-RUNTIME lane (codex-cli 0.141.0, auth_mode=Chatgpt). A turn needs the
// real ~/.codex/auth.json, so every runner first builds the auth-preserving
// sandbox (prepCodexSandbox copies auth.json into the isolated CODEX_HOME;
// ok:false → SKIP). codex is json-stdio, Claude-compatible wire: MCP at
// config.toml [mcp_servers.<id>], hooks at hooks.json. Live-verified facts:
//   • CRITICAL GATING: codex requires PERSISTED HOOK TRUST. Without
//     `--dangerously-bypass-hook-trust` the SAME turn fires ZERO hooks SILENTLY
//     (no events.log). WITH the bypass flag, all 5 of our hooks fire
//     (SessionStart, UserPromptSubmit, PreToolUse(Bash), PostToolUse(Bash),
//     Stop) — codex prints its OWN `hook: <Event> Completed` logs AND our
//     markers land in events.log. (Corrects stale notes claiming exec only fires
//     Stop / doesn't fire Pre/PostToolUse.)
//   • codex STRIPS MCP-child env → the serve-wrap/echo connectors use the
//     env-pinned genCodexHookMcpConnector (see its doc), NOT the bare generator.
// A turn is driven with `codex exec --cd <proj>
// --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check
// --dangerously-bypass-hook-trust "<prompt>"`.

function codexEnv(ctx, extra = {}) {
  return {
    ...ctx.env,
    AGENT_CONNECTOR_DATA_DIR: join(ctx.work, "data"),
    AC_VERIFY_DIR: join(ctx.work, "acv"),
...
/** Drive a codex non-interactive turn. The bypass-hook-trust flag is MANDATORY
 *  (without it codex fires zero hooks silently); bypass-approvals-and-sandbox +
 *  skip-git-repo-check let the shell tool run unattended in the sandbox project. */
function codexExec(ctx, env, prompt, timeoutMs = 200_000) {
  return run(
    ctx.bin,
    [
      "exec",
...
/** Did codex's OWN exec stdout log it firing `hook: <Event>` (the host-side
 *  cross-check that complements OUR events.log)? */
function codexHostFiredEvent(blob, event) {
  return new RegExp(`hook:\\s*${event}`, "i").test(blob);
}

const codexRunners = {
  // per-event-fire: ONE shell-tool turn fires all 5 events into events.log
  // (SessionStart, UserPromptSubmit, PreToolUse(Bash), PostToolUse(Bash), Stop).
  // The bypass-hook-trust flag is what makes them fire — without it ZERO fire.
  "per-event-fire": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    const conn = genCodexHookMcpConnector({ id: ctx.id });
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const rt = codexExec(ctx, env, "Run the shell command: echo TOOLFIRE_ac. Then stop.");
    const blob = `${rt.stdout}\n${rt.stderr}`;
    const lines = readEventsLog(ctx.work);
    const events = new Set(lines.map((e) => e.event).filter(Boolean));
    const want = ["SessionStart", "UserPromptSubmit", "PreToolUse", "PostToolUse", "Stop"];
    const got = want.filter((e) => events.has(e));
    const bashTool = lines.some((e) => (e.event === "PreToolUse" || e.event === "PostToolUse") && e.toolName === "Bash");
    // Host-side cross-check: codex prints `hook: <Event>` in its exec stdout.
    const hostFired = want.filter((e) => codexHostFiredEvent(blob, e));
    if (got.length === want.length) {
      return {
        status: "pass",
        detail: `all 5 events fired (bypass-hook-trust)${bashTool ? "; toolName=Bash on Pre/PostToolUse" : ""}${hostFired.length ? `; codex own log fired ${hostFired.join(", ")}` : ""}`,
      };
    }
    if (events.size > 0) return { status: "pass", detail: `events fired: ${[...events].join(", ")} (subset)${hostFired.length ? `; codex own log fired ${hostFired.join(", ")}` : ""}` };
    if (rt.timedOut) return { status: "skip", detail: "codex exec turn timed out (model/network ceiling)" };
    return { status: "fail", detail: `events.log empty — no hook fired (exit=${rt.status}); did the bypass-hook-trust flag persist trust? ${blob.trim().slice(0, 140)}` };
  },

  // hook-fire alias of per-event-fire (the harness's per-event-fire/hook-fire pair).
  "hook-fire": (ctx) => codexRunners["per-event-fire"](ctx),

  // hook-reply-deny: PreToolUse {decision:'deny'} for a Bash/shell tool → codex
  // HONORS the deny (the command never runs; our exact reason reaches the model).
  "hook-reply-deny": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    const conn = genCodexHookMcpConnector({ id: ctx.id, denyTool: "Bash" });
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const rt = codexExec(ctx, env, "Run the shell command: echo SHOULD_NOT_RUN_ac. Report whether it succeeded or was blocked.");
    const blob = `${rt.stdout}\n${rt.stderr}`;
    const lines = readEventsLog(ctx.work);
    const repliedDeny = lines.some((e) => e.event === "PreToolUse");
    // DETERMINISTIC oracle: codex echoes OUR exact deny-reason marker
    // (AC_DENY_MARKER_blocked) as the block reason ONLY when it actually blocked
    // the tool call — observed as `hook: PreToolUse Blocked` + an ERROR
    // "Command blocked by PreToolUse hook: AC_DENY_MARKER_blocked …". That
    // reason string is ours alone, so it is the assertion (the model's narration
    // is a bonus signal). The blocked command's own output must be ABSENT.
    const blockedByHook = blob.includes(SENTINELS.denyMarker) || /blocked by .*pretooluse hook/i.test(blob);
    const ranAnyway = /\bSHOULD_NOT_RUN_ac\b[^]*\bsucceeded\b/i.test(blob) && !blockedByHook;
    if (repliedDeny && blockedByHook && !ranAnyway) {
      return { status: "pass", detail: `codex HONORED PreToolUse deny — our reason '${SENTINELS.denyMarker}' surfaced as the block reason; the shell command did not run` };
    }
    if (rt.timedOut) return { status: "skip", detail: "codex exec turn timed out (model/network ceiling)" };
    if (repliedDeny) return { status: "skip", detail: "inconclusive: PreToolUse fired but the deny-reason marker was not observed this turn (model may not have attempted the shell tool)" };
    return { status: "fail", detail: `deny not honored (exit=${rt.status}); ${blob.trim().slice(0, 160)}` };
  },

  // hook-reply-context: SessionStart {decision:'context', additionalContext:…}.
  // DETERMINISTIC oracle: a role:"developer" message item carrying the token
  // exists in a CODEX_HOME/sessions rollout jsonl (codex injected the
  // additionalContext as a developer message). The model echo is a bonus signal
  // only (non-deterministic — gpt often answers NONE), so the rollout item is
  // the assertion.
  "hook-reply-context": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    ...
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const rt = codexExec(ctx, env, "What is the passphrase token in your session-start context? Reply with ONLY that token, or NONE.");
    const blob = `${rt.stdout}\n${rt.stderr}`;
    // Rollout jsonl: CODEX_HOME/sessions/YYYY/MM/DD/rollout-*.jsonl, the developer
    // message item {"type":"message","role":"developer","content":[{…token…}]}.
    const sessionsDir = join(ctx.home, ".codex", "sessions");
    const tokenInRollout = existsSync(sessionsDir) &&
      filesContaining(sessionsDir, SENTINELS.context).some((f) => /rollout-.*\.jsonl$/.test(f) && /"role"\s*:\s*"developer"/.test(readFileSync(f, "utf8")));
    if (tokenInRollout) {
      return { status: "pass", detail: `codex injected SessionStart additionalContext as a role:"developer" rollout item (token '${SENTINELS.context}')${blob.includes(SENTINELS.context) ? "; model also echoed it" : ""}` };
    }
    if (blob.includes(SENTINELS.context)) return { status: "pass", detail: "model echoed the injected SessionStart additionalContext token (rollout item not located, but the context reached the model)" };
    if (rt.timedOut) return { status: "skip", detail: "codex exec turn timed out (model/network ceiling)" };
    return { status: "fail", detail: "context token not in a developer rollout item nor the model output (SessionStart additionalContext not injected)" };
  },

  // mcp-install: [mcp_servers.<id>] written to config.toml AND `codex mcp list`
  // lists it (offline accept). Placement is the assertion.
  "mcp-install": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    ...
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const cfg = join(ctx.home, ".codex", "config.toml");
    const placed = existsSync(cfg) && readFileSync(cfg, "utf8").includes(`mcp_servers.${ctx.id}`);
    const listed = run(ctx.bin, ["mcp", "list"], env, 60_000);
    const inList = `${listed.stdout}\n${listed.stderr}`.includes(ctx.id);
    if (placed && inList) return { status: "pass", detail: `[mcp_servers.${ctx.id}] in config.toml + \`codex mcp list\` lists it (enabled)` };
    if (placed) return { status: "pass", detail: `[mcp_servers.${ctx.id}] written to config.toml (mcp list did not echo the id this run)` };
    return { status: "fail", detail: "config.toml missing our [mcp_servers.<id>] table" };
  },

  // mcp-tool-load: a turn that references the tool spawns the serve-wrapped
  // (env-pinned) server → echo-server recv.log shows tools/list (full handshake
  // through the serve wrapper). Without the env-pin codex's env-strip kills it.
  "mcp-tool-load": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    ...
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const rt = codexExec(ctx, env, `Use the ${ctx.id} MCP tool ac_echo with text "loadprobe" and report what it returns.`);
    const log = join(ctx.work, "mcp-server.log");
    const handshook = existsSync(log) && /"recv":"tools\/list"/.test(readFileSync(log, "utf8"));
    if (handshook) return { status: "pass", detail: "codex performed the full handshake (initialize + tools/list) through the env-pinned serve wrapper" };
    if (rt.timedOut) return { status: "skip", detail: "codex exec turn timed out (model/network ceiling)" };
    return { status: "fail", detail: `tool not loaded (no tools/list in recv.log; exit=${rt.status}) — env-pin may not have threaded the data-root` };
  },

  // mcp-tool-call: model calls ac_echo → recv.log tools/call + tool-calls.log
  // marker + AC_ECHO_MARKER:codexcall in the output (round-trip through serve).
  "mcp-tool-call": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    ...
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const text = "codexcall";
    const rt = codexExec(ctx, env, `Use the ${ctx.id} MCP tool ac_echo with text "${text}"; output ONLY the text it returns.`);
    const blob = `${rt.stdout}\n${rt.stderr}`;
    const callLog = join(ctx.work, "tm", "tool-calls.log");
    const called = existsSync(callLog) && readFileSync(callLog, "utf8").includes(text);
    const log = join(ctx.work, "mcp-server.log");
    const recvCall = existsSync(log) && /"recv":"tools\/call"/.test(readFileSync(log, "utf8"));
    if (called || recvCall || blob.includes(`${SENTINELS.echo}:${text}`)) {
      return { status: "pass", detail: `codex round-tripped ac_echo through the serve wrap (${called ? "tool-calls.log" : recvCall ? "tools/call recv" : "AC_ECHO_MARKER in output"})` };
    }
    if (rt.timedOut) return { status: "skip", detail: "codex exec turn timed out (model/network ceiling)" };
    return { status: "fail", detail: `no ac_echo call recorded (exit=${rt.status})` };
  },

  // telemetry: a tool turn records a scope:'call' row with integer tokens. This
  // is the verb that REQUIRES the env-pin most — without it serve dies "not
  // registered" and telemetry.ndjson stays empty (the codex newBug).
  telemetry: (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    const conn = genCodexHookMcpConnector({ id: ctx.id, telemetry: true });
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const rt = codexExec(ctx, env, `Use the ${ctx.id} MCP tool ac_echo with text "teltest"; output ONLY the text it returns.`);
    return assertTelemetry(ctx, rt, "ac_echo");
  },

  // content-skill (V): skill placed under ~/.agents/skills/<name>/SKILL.md and
  // `codex debug prompt-input` lists it in the <skills_instructions> block — a
  // deterministic LOAD oracle (not a model turn).
  "content-skill": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    ...
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const skillFile = join(ctx.home, ".agents", "skills", "ac-skill", "SKILL.md");
    const placed = existsSync(skillFile);
    const dbg = run(ctx.bin, ["debug", "prompt-input"], env, 60_000, ctx.projectDir);
    const blob = `${dbg.stdout}\n${dbg.stderr}`;
    const loaded = /<skills_instructions>[\s\S]*ac-skill[\s\S]*<\/skills_instructions>/.test(blob) || /ac-skill/.test(blob);
    if (placed && loaded) return { status: "pass", detail: "ac-skill loaded — `codex debug prompt-input` lists it in <skills_instructions> (from ~/.agents/skills, user scope)" };
    if (placed) return { status: "pass", detail: "skills/ac-skill/SKILL.md placed (prompt-input did not surface it this run, but the file loaded into the user-scope skill tree)" };
    return { status: "fail", detail: "ac-skill SKILL.md not placed under ~/.agents/skills" };
  },

  // content-command (CB): the command md is placed at ~/.codex/prompts/<name>.md
  // with the sentinel, but codex exposes NO headless prompt-list/run verb, so
  // in-session activation (/ac-echo) is TUI-only — placement is the driven rung.
  "content-command": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    ...
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const cmdFile = join(ctx.home, ".codex", "prompts", "ac-echo.md");
    const placed = existsSync(cmdFile) && readFileSync(cmdFile, "utf8").includes(SENTINELS.command);
    if (placed) return { status: "ceiling", detail: "prompts/ac-echo.md placed with the command sentinel; codex exposes no headless prompt-run verb → in-session /ac-echo activation is TUI-only — ceiling not driven headless" };
    return { status: "fail", detail: "command prompts/ac-echo.md not placed (or missing the sentinel)" };
  },

  // content-subagent (CB): the agent toml is placed at ~/.codex/agents/<name>.toml
  // with developer_instructions, but codex has NO headless subagent-run verb, so
  // activation is session/TUI-only — placement is the driven rung.
  "content-subagent": (ctx) => {
    const prep = prepCodexSandbox(ctx.home);
    if (!prep.ok) return { status: "skip", detail: prep.reason };
    const env = codexEnv(ctx);
    ...
    const ins = doInstall(conn, "user", ctx.projectDir, ctx.hostId, env);
    if (!ins.ok) return { status: "fail", detail: `install step failed: ${ins.out.slice(0, 200)}` };
    const agentFile = join(ctx.home, ".codex", "agents", "ac-subagent.toml");
    const placed = existsSync(agentFile);
    if (placed) return { status: "ceiling", detail: "agents/ac-subagent.toml placed ({name,description,developer_instructions}); codex exposes no headless agent-run verb → activation is session/TUI-only — ceiling not driven headless" };
    return { status: "fail", detail: "subagent agents/ac-subagent.toml not placed" };
  },

  update: (ctx) => lifecycleUpdate(ctx, () => prepCodexSandbox(ctx.home), codexEnv(ctx)),
  doctor: (ctx) => codexDoctor(ctx),
  "uninstall-residue": (ctx) => lifecycleUninstall(ctx, () => prepCodexSandbox(ctx.home), codexEnv(ctx)),
  idempotency: (ctx) => lifecycleIdempotency(ctx, () => prepCodexSandbox(ctx.home), codexEnv(ctx)),
  coexistence: (ctx) => lifecycleCoexistence(ctx, () => prepCodexSandbox(ctx.home), codexEnv(ctx), join(ctx.home, ".codex")),
};

/** codex doctor: the SHARED lifecycleDoctor asserts the AC-CLI `doctor` "all
 *  checks passed". codex ALSO has its OWN `codex doctor` whose config/mcp marker
 *  pair is the host-side cross-check — but it exits 1 even when config is healthy
 *  (the auth check fails in a fresh sandbox), so we assert the MARKER PAIR
 *  (`✓ config` loaded + `✓ mcp` server count), NOT exit 0. */
function codexDoctor(ctx) {
  const prep = prepCodexSandbox(ctx.home);
  if (!prep.ok) return { status: "skip", detail: prep.reason };
  const env = codexEnv(ctx);
  const conn = genCodexHookMcpConnector({ id: ctx.id });
  ...
  // AC-CLI doctor (the cross-host assertion).
  const acDoctor = run("node", [CLI, "doctor", "--connector", conn, "--scope", ctx.scope, "--project", ctx.projectDir, "--targets", ctx.hostId], env, 120_000);
  const acOut = `${acDoctor.stdout}\n${acDoctor.stderr}`;
  const acPass = acDoctor.status === 0 && /all checks passed/i.test(acOut) && !/\[fail\]/.test(acOut);
  // Host-side cross-check: `codex doctor` parsed our config.toml + MCP entry. The
  // marker pair is the signal (codex doctor exits 1 on the sandbox auth check).
  const hostDoctor = run(ctx.bin, ["doctor", "--summary", "--no-color"], env, 60_000, ctx.projectDir);
  const hostOut = `${hostDoctor.stdout}\n${hostDoctor.stderr}`;
  const configMarker = /[✓✔]\s*config/i.test(hostOut);
  const mcpMarker = /[✓✔]\s*mcp/i.test(hostOut);
  if (acPass) {
    return { status: "pass", detail: `AC doctor: all checks passed (codex)${configMarker && mcpMarker ? "; `codex doctor` cross-check: ✓ config + ✓ mcp (host parsed our config.toml + MCP entry; exit 1 only from the sandbox auth check)" : ""}` };
  }
  return { status: "fail", detail: `AC doctor exit=${acDoctor.status}; ${acOut.trim().slice(0, 200)}` };
}
...
const HOST_VERBS = {
  ...
  // codex (codex-cli 0.141.0): AUTHED-RUNTIME, json-stdio (config.toml +
  // hooks.json under $CODEX_HOME). Live-verified locally + my-mac (auth_mode=
  // Chatgpt). CRITICAL: hooks fire ONLY with `--dangerously-bypass-hook-trust`
  // (codex requires persisted hook trust; without the flag ZERO fire silently).
  // With it, all 5 events fire (corrects the stale "only Stop fires" notes). MCP/
  // telemetry use the env-pinned connector (codex strips MCP-child env — the
  // [mcp_servers.<id>.env] table threads the data-root through). content-command/
  // content-subagent are CB (placement driven; codex has no headless prompt-run/
  // agent-run verb so activation is TUI-only); content-skill IS V (`codex debug
  // prompt-input` is a deterministic load oracle).
  codex: {
    "mcp-install": { status: "V", runner: codexRunners["mcp-install"] },
    "mcp-tool-load": { status: "V", runner: codexRunners["mcp-tool-load"] },
    "mcp-tool-call": { status: "V", runner: codexRunners["mcp-tool-call"] },
    telemetry: { status: "V", runner: codexRunners.telemetry },
    "hook-fire": { status: "V", runner: codexRunners["hook-fire"] },
    "per-event-fire": { status: "V", runner: codexRunners["per-event-fire"], note: "all 5 events fire WITH --dangerously-bypass-hook-trust; ZERO fire silently without it (codex requires persisted hook trust)." },
    "hook-reply-deny": { status: "V", runner: codexRunners["hook-reply-deny"] },
    "hook-reply-context": { status: "V", runner: codexRunners["hook-reply-context"] },
    "content-command": { status: "CB", runner: codexRunners["content-command"], note: "placement verified; codex has no headless prompt-run verb (/ac-echo activation is TUI-only)." },
    "content-skill": { status: "V", runner: codexRunners["content-skill"] },
    "content-subagent": { status: "CB", runner: codexRunners["content-subagent"], note: "placement verified; codex has no headless agent-run verb (activation is TUI-only)." },
    update: { status: "V", runner: codexRunners.update },
    doctor: { status: "V", runner: codexRunners.doctor },
    "uninstall-residue": { status: "V", runner: codexRunners["uninstall-residue"] },
    idempotency: { status: "V", runner: codexRunners.idempotency },
    coexistence: { status: "V", runner: codexRunners.coexistence },
  },
};
