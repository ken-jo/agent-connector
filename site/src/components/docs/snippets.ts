...
export const defineConnectorSnippet = `import { defineConnector } from "agentconnect";
...
export const brandedPackageJsonSnippet = `{
  ...
  "files": ["bin.mjs", "agentconnect.config.mjs"],
  "dependencies": {
    "agentconnect": "^0.1.0"
  }
}`;
 ...
 * exposes every agentconnect subcommand under your brand, AUTO-SCOPED to the
 ...
 * without --connector: serve/hook still route through the one ~/.agentconnect
...
# agentconnect itself is never installed globally.
 ...
 * agentconnect command with the developer's connector injected.
 */
export const brandedScopingSnippet = `# a branded command  ≈  the agentconnect command, connector pre-injected:
acme-db install        ≈  agentconnect install --connector ./agentconnect.config.mjs
acme-db leaderboard    ≈  agentconnect leaderboard --connector acme-db
acme-db telemetry report --by tool
                       ≈  agentconnect telemetry report --by tool --connector acme-db
...
export const leaderboardSnippet = `$ agentconnect leaderboard --since 7d
 ...
 * `agentconnect leaderboard --connector acme-db`, so a connector developer
...
#  ≈  agentconnect leaderboard --connector acme-db --since 7d
...
#  ≈  agentconnect telemetry leaderboard --by tool --connector acme-db
...
agentconnect package --format gemini-extension --out ./dist --dry-run

# emit EVERY feasible format, each into <out>/<format>/
agentconnect package --format all --out ./dist

# an unknown format exits 2
agentconnect package --format bogus   # → invalid --format "bogus" (exit 2)`;
...
/plugin install my-connector@agentconnect

# the wrapped MCP entry in the bundle still routes through the home-bin:
#   agentconnect serve --connector my-connector --host claude-code -- <real cmd>
# so a marketplace-installed connector STILL reports per-tool tokens.`;

export const surfaceLeaderboardSnippet = `$ agentconnect telemetry leaderboard --by surface
...
export const operatingModelSnippet = `~/.agentconnect/
  bin/agentconnect              single binary: CLI + hook entrypoint + telemetry
