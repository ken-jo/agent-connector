...
export const memoryTargetRows: {
  ...
  {
    host: "33 AGENTS.md adopter hosts",
    project:
...
export const surfaceSupportRows: {
  ...
  },
  {
    ...
    skill: ".kilo/skills/<n>/SKILL.md",
    subagent: ".kilocode/agents/<n>.md",
  },
  {
    platform: "pi",
    command: ".pi/prompts/<n>.md",
...
export const tsPluginPlatforms: PlatformEntry[] = [
  ...
  },
  {
    name: "MiMoCode",
    id: "mimo-code",
    target: "OpenCode fork (Xiaomi @mimo-ai/cli); generated plugin module in ~/.config/mimocode/plugin/ (MCP root key mcp)",
  },
  {
    name: "Kilo CLI",
    id: "kilo-cli",
  ...
  },
  {
    name: "Kilo Code",
    id: "kilo",
    target: "generated plugin module in .kilo/plugin/ (7.x shares the Kilo CLI backend)",
  },
...
/**
 * The 13 PackageFormat values (11 host formats + 2 opt-in standard artifacts),
...
export const packageFormatRows: PackageFormatRow[] = [
  ...
  {
    format: "agent-plugin",
    targets: "Agent Plugins 1.0.0 clients (portable): VS Code · Cursor · GitHub Copilot · ChatGPT/Codex · Kiro · Hermes · OpenClaw · …",
    manifest: "plugin.json ($schema + name; root) + mcp.json ($schema + mcpServers: stdio | streamable-http | sse) + skills/<n>/SKILL.md + com.github.copilot/{hooks/hooks.json, commands/, agents/} + README.md",
    install: "push <out>/<id> to a git repo, then install from source in the client (VS Code: “Chat: Install Plugin From Source” → repo URL; others per https://agent-plugins.org/compatible-clients). Consumer prerequisite: npm i -g @ken-jo/agent-connector.",
    note: "The open, vendor-neutral spec (agent-plugins.org — Vercel-led with AWS, Cursor, GitHub, Microsoft, OpenAI). Portable core = skills + MCP; hooks/commands/subagents are client-specific and ride in the com.github.copilot/ extension namespace (VS Code + Copilot read it, others ignore it). NO absolute path embedded (the spec forbids it): the serve-wrapper + hooks invoke the bare `agent-connector` bin. The only bundle that also carries REMOTE servers (streamable-http / sse).",
  },
  {
