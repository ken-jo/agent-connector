...
export const platforms: PlatformHookEntry[] = [
  ...
  },
  {
    ...
    paradigm: "json-stdio",
    hasHooks: true,
    configPath: "$INTERPRETER_HOME|~/.openinterpreter/hooks.json · <project>/.openinterpreter/hooks.json (Claude-shaped {matcher,hooks[]})",
    capabilities: {
      canModifyArgs: true,
      canModifyOutput: false,
      canInjectSessionContext: true,
    ...
    notes:
      "Open Interpreter — the new Rust `interpreter`/`i` CLI, a FORK of OpenAI's Codex, so the adapter EXTENDS the codex adapter: same Claude-shaped hooks.json and hookSpecificOutput replies, under $INTERPRETER_HOME (default ~/.openinterpreter; $CODEX_HOME is ignored) and a `.openinterpreter` project dir. `features.hooks` is stable and on by default. VERIFIED 2026-09-07 by source (codex-rs/hooks, docs/hooks.md) and a live run against a mock provider that fired SessionStart, UserPromptSubmit, PreToolUse (tool_name Bash), PostToolUse, Stop and SessionEnd — SessionEnd is registered here on top of CODEX_HOOK_EVENTS (host clamps its timeout to 3s). Trust gate as upstream Codex: non-managed command hooks run after `/hooks` review or `--dangerously-bypass-hook-trust`. No Notification / PostToolUseFailure analog.",
  },
  {
    platform: "junie",
    displayName: "Junie",
