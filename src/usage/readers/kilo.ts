 ...
 * globalStorage:
 *   <vscodeUserDir>/globalStorage/kilocode.kilo-code/tasks/<taskId>/
 *     ui_messages.json               (the UI message stream — token source)
 *     api_conversation_history.json  (sibling — model/agent source)
 * The Rust scanner additionally discovers a remote-host server path under
 * ~/.vscode-server/data/User/globalStorage/kilocode.kilo-code/tasks, so we scan
 * both the desktop and server tasks roots that exist.
 *
 ...
 *   cost (f64)   → cost
 *   apiProtocol  → providerId (preserved verbatim, e.g. "azure/openai")
 * reasoning is ALWAYS 0 (Kilo Code does not report it). A malformed text payload,
 ...
 * apiProtocol (trimmed, default "unknown"). No project attribution (Kilo Code's
 * logs carry no cwd). No dedup — one row per api_req_started event (file
 * isolation is the implicit unique context). Confidence is "host-reported"
 * (real host-logged token counts; the design rates this medium qualitatively).
 *
...
const PLATFORM_ID = "kilo" as const;
...
function tasksRoots(): string[] {
  ...
  const home = homedir();
  return [
    join(vscodeUserDir(), "globalStorage", KILO_EXTENSION_ID, "tasks"),
    join(home, ".vscode-server", "data", "User", "globalStorage", KILO_EXTENSION_ID, "tasks"),
  ];
}
...
// ─────────────────────────────────────────────────────────────────────────
// File parse (port of parse_roo_kilo_file with source "kilocode")
// ─────────────────────────────────────────────────────────────────────────
...
function parseKiloFile(path: string): UsageRecord[] {
  ...
  for (const raw of data) {
    ...
    // reasoning stays 0 (Kilo Code does not report it).
...
/** The Kilo Code (VS Code extension) usage reader singleton. */
const kiloReader: UsageReader = {
  platformId: PLATFORM_ID,
  kind: "local",
  async read({ sinceMs }: { sinceMs?: number }): Promise<UsageRecord[]> {
    const roots = tasksRoots().filter((root) => existsSync(root));
    if (roots.length === 0) return []; // no tasks dir → fail-open
