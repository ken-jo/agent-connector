/**
 * adapters/omp — Oh My Pi (OMP) platform adapter for agentconnect.
 *
 ...
 *   that imports NOTHING from agentconnect: each `pi.on(...)` handler shells
...
export class OMPAdapter extends BaseAdapter implements Adapter {
   ...
   *      agentconnect. Its default export is the OMP HookFactory `(pi)=>void`;
  ...
  private buildManifest(ctx: InstallContext): string {
    const manifest = {
      ...
      description: `${ctx.connector.displayName} OMP extension (generated by agentconnect)`,
      type: "module",
      main: "index.js",
