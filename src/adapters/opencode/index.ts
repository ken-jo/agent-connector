/**
 * adapters/opencode — OpenCode (SST) platform adapter for agentconnect.
 *
 ...
 *   code shipped with the package. agentconnect cannot: the connector's hook
...
export class OpenCodeAdapter extends BaseAdapter implements Adapter {
   ...
   * wires. Only events present in EVENT_TO_OPENCODE are mapped/wired; any declared
   * event with no OpenCode mapping (e.g. UserPromptSubmit) is reported separately
   ...
   *
   * The module imports nothing from agentconnect. It embeds two constants
  ...
  private buildPluginSource(ctx: InstallContext): string {
    ...
    const header = `/**
 * AUTO-GENERATED by agentconnect — DO NOT EDIT.
 *
 * Self-contained OpenCode plugin bridge for connector ${ctx.connector.id}.
 * It imports nothing from agentconnect: every hook invocation shells out to
...
function bridge(event, payload) {
  try {
    // On Windows HOME_BIN is the agentconnect.cmd launcher: Node cannot
