/**
 * adapters/openclaw — OpenClaw (Gateway) platform adapter for agentconnect.
 *
 ...
 *   shipped with the package. agentconnect cannot: the connector's hook
 ...
 *   register(api) } — that imports NOTHING from agentconnect and, on each
...
export class OpenClawAdapter extends BaseAdapter implements Adapter {
  ...
  private buildPluginSource(ctx: InstallContext): string {
    ...
    const header =
      ...
      ctx.connector.id +
      ".\n" +
      " * It imports nothing from agentconnect: every hook invocation shells out\n" +
    ...
    reg.push(
      "  register(api) {\n" +
        ...
        "        try {\n" +
        '          api.registerHook(event, handler, { name: CONNECTOR_ID + ":" + event, description: "agentconnect bridge" });\n' +
        "        } catch { /* best effort */ }\n" +
        "      }\n" +
        "    };\n",
    );
