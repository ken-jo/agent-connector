...
function buildPackageJson(connector: ResolvedConnector): Record<string, unknown> {
  ...
  return {
    ...
    main: "index.js",
    exports: { ".": "./index.js" },
    keywords: ["opencode", "opencode-plugin", "pi-package", "agentconnect"],
...
function buildIndexJs(connector: ResolvedConnector): string {
  const homeBin = JSON.stringify("agentconnect");
 ...
 * nothing from agentconnect: every hook invocation shells out to the stable
...
function bridge(event, payload) {
  try {
    ...
    // PATH, where on Windows it is agentconnect.cmd. Node cannot execFile a
