 ...
 *   - "json-stdio" (24): host pipes JSON to a command on stdin, reads
 ...
 *     the agy CLI), Continue, Amazon Q, Grok Build, Grok CLI, Devin, Open
 *     Interpreter (a Codex fork). One universal entrypoint binary handles all of
 *     them.
 ...
 *   - "mcp-only" (10): no hook layer at all — Warp, Cline, Trae, Zed, Freebuff,
 *     Xum, Pi, Windsurf, Junie, Mistral Vibe. Only the MCP
 *     server (or skills surface) is installed; hooks are reported unavailable.
 */
 ...
 * standard (29/35 hosts), the host's own file (CLAUDE.md / GEMINI.md)
...
export interface StatuslineContext extends HostCtx {
   ...
   * exposed as a synchronous property. The runtime installs it lazily, so the
   * all-time NDJSON scan happens only if render() reads `ctx.usage`. ALL-TIME
   * for the connector (every stored row) — NOT the host context-window (see
   * `context`) and NOT session-scoped. When runtime-provided, first read returns
   * a real summary (zeros when there's no telemetry or it's switched off, never
   * undefined); it is undefined only for a context that does not provide it.
   * render() can still `await ctx.telemetry?.()` to fetch the same summary via
   * the async accessor.
   */
