/**
 * adapters/zed-skills — skills surface tests for the Zed adapter.
 *
 * Zed reads SKILL.md from:
 ...
 *   - platforms['zed'].skills === false disables the surface
 */
...
function freshProject(): string {
  const dir = mkdtempSync(join(tmpdir(), "ac-zed-skills-"));
...
describe("zed adapter — skills surface", () => {
  ...
  it("user-scope skill does NOT write into the project .agents tree", () => {
    ...
    const projDir = mkdtempSync(join(tmpdir(), "ac-zed-skills-proj-"));
